...
const COMPANY = COMMANDE_COMPANY_ASV;

// ── Form helpers ──────────────────────────────────────────────────────────────
function inputCls(err?: string) {
  return `w-full px-3 py-3 rounded-xl border text-base sm:text-sm bg-white focus:outline-none focus:ring-2 focus:ring-[#facc15] ${err ? "border-red-300" : "border-gray-200"}`;
}
...
// ── Dashboard (form-only) ─────────────────────────────────────────────────────
export default function CommandesAsvDashboard() {
  const [form, setForm] = useState<Commande>(() => emptyCommande(COMPANY));
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const fRes = await fetch("/api/commandes/fournisseurs");
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const toSave = { ...form, company: COMPANY };
      if (devisFile) {
      ...
      if (fName && !fournisseurs.includes(fName)) {
        setFournisseurs((prev) =>
          [...prev, fName].sort((a, b) => a.localeCompare(b, "fr-CH", { sensitivity: "base" }))
        );
      }
    ...
    <div className="min-h-screen bg-warm">
      <header className="glass sticky top-0 z-20">
        <div className="max-w-xl mx-auto px-3 sm:px-5 py-2 sm:py-3 flex items-center justify-between gap-2">
          <NavButton href="/asv-fenetres" label="Retour" />
      ...
      <main className="max-w-xl mx-auto px-5 py-6 sm:py-8">
        <div className="text-center text-[11px] text-[#6b7280] uppercase tracking-wide mb-4">
          {COMPANY}
        </div>
        <form onSubmit={handleSubmit} className="glass-card rounded-2xl p-5 sm:p-6 space-y-4">
          ...
          <FormField label="Chantier / Référence *" error={errors.chantier}>
            <input
              type="text"
              value={form.chantier}
              onChange={(e) => set("chantier", e.target.value)}
              placeholder="Nom du chantier ou référence"
          ...
          >
            ...
            />
            <datalist id="fournisseurs-asv-list">
              {fournisseurs.map((f) => (
          ...
          >
            <input
              ...
              placeholder="Fenêtres PVC triple vitrage, 6 pièces"
          ...
          <button
            type="submit"
            disabled={saving}
            className="w-full btn-mba text-base py-3.5 rounded-xl disabled:opacity-50 disabled:cursor-not-allowed"
          >
